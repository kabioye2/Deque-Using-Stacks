...
//            to.push (from.pop ());
        ...
        //put half of stack in tmp stack
        	tmp.push(from.pop());
        }
        //if one side empty, move to other side
        //if 'to' stack not empty, then exception is thrown so 'to' is empty
        //if 'from' is empty, then exception thrown so 'from' is not empty
      
        //while 'from' is not empty, pop contents of 'from' to 'to'
        //now you can pop from the bottom
        while(!from.isEmpty()) {
        	to.push(from.pop());
        }
        
      //while 'tmp' is not empty, pop contents of 'tmp' to 'from'
       //can still pop from the top
        while(!tmp.isEmpty()) {
        	from.push(tmp.pop());
        }
